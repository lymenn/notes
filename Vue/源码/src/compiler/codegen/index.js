...
export function generate (
    ...
    // children为所有子节点的字符串格式的代码组成的字符串数组，格式：
    //     `['_c(tag, data, children)', ...],normalizationType`，
    //     最后的normalizationType为_c的第四个参数，
    //     表示节点的规范化类型，不是重点，不需要关注
    // 当然code并不一定是_c,也有可能是其他的，比如整个组件都是静态的，则结果为_m(0)
...
export function genElement (el: ASTElement, state: CodegenState): string {
    ...
    if (el.staticRoot && !el.staticProcessed) {
        ...
        // 处理带有v-if指令的节点，最终得到一个三元表达式：condition ? render1 : render2
        return genIf(el, state)
    } else if (el.tag === 'template' && !el.slotTarget && !state.pre) {
        // 当前节点不是template标签也不是插槽和带有v-pre指令的节点时走这里
...
function genStatic (el: ASTElement, state: CodegenState): string {
    ...
    // 将静态根节点的渲染函数push到staticRenderFns数组中， 比如：
...
// v-once
// 处理带有v - once的节点，结果会有三种:
// 1、当节点存在v - if指令，得到一个三元表达式，condition ？ render1：render2
// 2、当节点是一个包含在v - for指令内部的静态节点，得到`_o(_c(tag, data, children), number, key)`
// 3、当前节点就是一个单纯的 v-once 节点，得到 `_m(idx, true of '')`
function genOnce (el: ASTElement, state: CodegenState): string {
    // 标记当前的v-once指令已经被处理过了
    el.onceProcessed = true
    if (el.if && !el.ifProcessed) {
        // 如果有v - if指令 & if指令没有被处理，则走这里
        // 处理带有v-if指令的节点，最终得到一个三元表达式，condition ？ render1 ： render2
        return genIf(el, state)
    } else if (el.staticInFor) {
        // 说明当前节点是被包裹在还有v - for指令内部的静态节点
        // 获取v-for指令的key
        let key = ''
        ...
        if (!key) {
            // key不存在给出提示，v-once节点只能用于带有key的v-for节点内部
...
function genIfConditions (
    ...
    // 从conditions数组中拿出第一个条件对象{ exp: block}
    const condition = conditions.shift()
    // 返回一个三元表达式字符串，condition ？ 渲染函数1： 渲染函数2
    if (condition.exp) {
        // 如果condition.exp条件成立，则得到一个三元表达式
        // 如果条件不成立，则通过递归的方式找conditions数组中的下一个元素
        // 知道条件成立的元素，返回一个三元表达式
...
// 处理节点上的众多属性，最后生成这些节点组成的JSON字符串，比如 data = { key: xx, ref: xx, ... } 
export function genData (el: ASTElement, state: CodegenState): string {
    ...
    // 执行指令的编译方法，比如web平台的v-text,v-html,v-model，然后el对象上添加相应的属性
    // 比如v - text: el.textContent = _s(value, dir)
    // v - html: el.innerHTML = _s(value, dir)
    // 当前指令在运行时还有任务，比如v - model，则返回directives: [{ name, rawName, value, arg, modifiers }, ...}] 
    ...
    // 带有ref属性的节点，在带有v-for指令节点的内部，data = { refInFor: true }
    if (el.refInFor) {
        data += `refInFor:true,`
    }
...
// 运行指令的编译方法，如果指令存在运行时任务，则返回directives: [{ name, rawName, value, arg, modifiers }, ...}] 
function genDirectives (el: ASTElement, state: CodegenState): string | void {
    ...
    // 指令的处理结果
    let res = 'directives:['
    // 标记，用于标记指令是否需要在运行时完成任务，比如v-model的input事件
    ...
    for (i = 0, l = dirs.length; i < l; i++) {
        dir = dirs[i]
        needRuntime = true
        // 获取节点当前指令的处理方法，比如web平台的v-html、v-text、v-model
        const gen: DirectiveFunction = state.directives[dir.name]
        if (gen) {
            // compile-time directive that manipulates AST.
            // returns true if it also needs a runtime counterpart.
            // 执行指令的编译方法，如果指令还需要在运行时完成一部分任务，则返回true，比如v-model
...
export function genChildren (
    ...
    if (children.length) {
        ...
        if (children.length === 1 &&
            ...
            // 优化，只有一个子节点&&子节点上有v-for指令&&子节点的标签不为template或者slot
            // 优化的方式是直接调用genElement生成该节点的渲染函数，不需要走下面的循环然后调用genCode最后得到渲染函数
            const normalizationType = checkSkip
                ? state.maybeComponent(el) ? `,1` : `,0`
                : ``
            return `${(altGenElement || genElement)(el, state)}${normalizationType}`
        }
        // 获取节点规范化类型，返回一个number 0、1、2，不是重点
        const normalizationType = checkSkip
