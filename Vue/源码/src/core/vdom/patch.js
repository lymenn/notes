...
function sameVnode (a, b) {
    return (
        // 如果a,b都没有设置key的话，则a.key === b.key
        // 没有设置key的话，默认两个节点的key是一致的
        // 需要注意的是 undefined === undefined => true
        a.key === b.key && (
            (
                // 标签相同
                a.tag === b.tag &&
...
// 工厂函数，注入平台特有的一些功能操作，定义一些方法，然后返回patch函数
export function createPatchFunction (backend) {
    ...
    function createElm (
        ...
        if (isDef(tag)) {
            ...
            if (__WEEX__) {
                ...
                // 将创建好的vnode插入到parent中，如果relElm存在的话，就插入到relElm元素之前
                // 将节点插入父节点
                insert(parentElm, vnode.elm, refElm)
            }
    ...
    function updateChildren (parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
        // 旧前索引：oldCh中所有未处理的第一个节点的下标
        let oldStartIdx = 0
        // 新前索引：newCh中所有未处理的第一个节点的下标
        let newStartIdx = 0
        // 旧后索引：oldCh中所有未处理的最后一个节点下标
        let oldEndIdx = oldCh.length - 1
        // 旧前节点
        let oldStartVnode = oldCh[0]
        // 旧后节点
        let oldEndVnode = oldCh[oldEndIdx]
        // 新后索引：newCh中所有未处理的最后一个节点下标
        ...
        // 当开始位置大于结束位置时，说明 oldCh或newCh中的节点已经遍历结束
        // 当新子节点和旧子节点的数量不一致时，会导致循环结束后仍有未处理的节点
        // 如果oldCh先循环结束，说明newCh中还有剩余节点，说明这些节点都是需要新增的节点，直接把这些节点插入dom中，不需要要循环对比了
        // 如果newCh先循环结束，说明oldCh中还有剩余节点，说明这些节点都是需要删除的节点，直接把这些节点从dom中删除，不需要循环对比了
        // 跳出循环，就可以直接判断出差异，减少了循环，提升了性能
        // 即在oldStartIdx和oldEndIdx，newStartIdx和newEndIdx之间的所有节点都是未处理的节点
        while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {

            if (isUndef(oldStartVnode)) {
                // 旧前节点已经被移动,oldStartVnode设置为下一个节点
                // 移动节点时，真正移动的是DOM节点，为了防止后续重复处理同一个节点，旧的虚拟节点会被设置为undefined
                // 用来标记这个节点已经被处理过且移动到其他位置
                oldStartVnode = oldCh[++oldStartIdx] // Vnode has been moved left
            } else if (isUndef(oldEndVnode)) {
                // 旧后节点已经被移动,oldEndVnode设置为前一个节点
                oldEndVnode = oldCh[--oldEndIdx]
            } else if (sameVnode(oldStartVnode, newStartVnode)) {
                // 新前和旧前(oldStartVnode和newStartVnode)是同一个节点
                // 更新节点
                ...
                // 旧后与新后（oldEndVnode与newEndVnode）是同一个节点
                // 更新节点
                ...
                // 四种情况都没有匹配成功
                // 建立key与index的对应关系 返回一个hashmap {key1: index1, key2:index2}
                if (isUndef(oldKeyToIdx)) oldKeyToIdx = createKeyToOldIdx(oldCh, oldStartIdx, oldEndIdx)
                // idxInOld表示oldVnode.children中与newStartVnode是同一个节点的那个节点的索引值
                idxInOld = isDef(newStartVnode.key)
                    // 存在newStartVnode.key 根据索引关系，直接通过key获取索引值。避免了后面的循环，提高了效率
                    ? oldKeyToIdx[newStartVnode.key]
                    // 没有设置newStartVnode.key，则只能循环oldChildren，通过sameVnode方法找到相同的那个节点的索引值
                    : findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx)
                if (isUndef(idxInOld)) { // New element
                    // 找不到相同的节点
                    // 使用新前节点创建新节点插入到旧前节点的前面
                    createElm(newStartVnode, insertedVnodeQueue, parentElm, oldStartVnode.elm, false, newCh, newStartIdx)
                } else {
                    vnodeToMove = oldCh[idxInOld]
                    if (sameVnode(vnodeToMove, newStartVnode)) {
                        // 是同一个节点
                        // 对这两个节点进行patchVnode操作
                        patchVnode(vnodeToMove, newStartVnode, insertedVnodeQueue, newCh, newStartIdx)
                        // 在oldChildren中清空找到的节点，标记为undefined
                        oldCh[idxInOld] = undefined
                        // 把找到的节点移动到旧前节点的前面
                        canMove && nodeOps.insertBefore(parentElm, vnodeToMove.elm, oldStartVnode.elm)
                    } else {
                        // same key but different element. treat as new element
                        // 相同的key但不是同一个元素，使用新前节点创建节点并插入到旧前节点的前面
                        createElm(newStartVnode, insertedVnodeQueue, parentElm, oldStartVnode.elm, false, newCh, newStartIdx)
                    }
                }
                // 将新前节点设置为下一个节点
        ...
        if (oldStartIdx > oldEndIdx) {
            // 旧的子节点先遍历完，添加新的子节点中的剩余节点到parentElm中
            refElm = isUndef(newCh[newEndIdx + 1]) ? null : newCh[newEndIdx + 1].elm
            addVnodes(parentElm, refElm, newCh, newStartIdx, newEndIdx, insertedVnodeQueue)
        } else if (newStartIdx > newEndIdx) {
            // 新的的子节点先遍历完，删除旧的子节点中的没被遍历的剩余节点
    ...
    // @param ownerArray 节点数组
    // @param index 当前节点索引
    // @param removeOnly 旧节点
    // 【vnode、oldVnode完全一样？】→ 是 → 退出程序
    //  ↓
    //  否
    //  ↓
    // 【vnode与oldVnode是静态节点？】是 退出程序
    //  ↓
    //  否
    //  ↓
    // 【vnode有text属性？】→ 是 → 【vnode与oldVnodet的文本不同？】→ 是 →  用vnode的文本替换真实dom中的文本
    //  ↓
    //  否
    //  ↓
    // 【vnode的子节点和oldVnode子节点都存在】 → 是 →  【vnode与oldVnode子节点不同？】→ 是 →  更新子节点 updateChildren
    //  ↓
    //  否
    //  ↓
    // 【只有vnode的子节点存在？】→ 是 → 【oldVnode有文本，清空DOM中的文本】把vnode的子节点添加到DOM中
    //  ↓
    //  否
    //  ↓
    // 【只有oldVnode的子节点存在？】 → 是 →  清空DOM中的子节点
    //  ↓
    //  否
    //  ↓
    // 【oldVnode中有文本？】→ 是 →  清空DOM中的文本
    /**
     * 更新节点
     *   全量的属性更新
    ...
    function patchVnode (
        ...
        if (isUndef(vnode.text)) {
            ...
            if (isDef(oldCh) && isDef(ch)) {
                //vnode和oldVnode都有子节点，且子节点不同，调用updateChildren进行深度对比，更新子节点
                // 如果新老节点都有孩子，则递归执行 diff 过程
                if (oldCh !== ch) updateChildren(elm, oldCh, ch, insertedVnodeQueue, removeOnly)
            } else if (isDef(ch)) {
                // vnode中有子节点，oldVnode中没有
                if (process.env.NODE_ENV !== 'production') {
            ...
            // vnode有text属性，且与oldVnode的文本不相同，则把真实DOM节点中的文本改为vnode.text所保存的文本
            nodeOps.setTextContent(elm, vnode.text)
        }
        if (isDef(data)) {
            if (isDef(i = data.hook) && isDef(i = i.postpatch)) i(oldVnode, vnode)
        }
        // vnode和oldVnode的文本相同， 退出程序
    }
    ...
    return function patch (oldVnode, vnode, hydrating, removeOnly) {
        ...
        if (isUndef(oldVnode)) {
            ...
            if (!isRealElement && sameVnode(oldVnode, vnode)) {
                ...
                // 当两个节点相同时，进入patchVnode,patchVnode中如果两个节点都有相同的子节点会进一步调用updateChildren深度对比
